...
inline static int serial_init(int ndev, int baudrate)
{
...
inline static int serial_read(int ndev)
{
	return bUART(ndev, oURXHL);
}

inline static int serial_write(int ndev, int ch)
{
...
inline static int serial_poll(int ndev)
{
	return ((bUART(ndev, oUFSTAT) & UFSTAT_RX_CNT) != 0);
}

inline static int serial_flush_input(int ndev)
{
	int tmp;
	while(bUART(ndev, oUFSTAT) & UFSTAT_RX_CNT)
		tmp=bUART(ndev, oURXHL);
	if (tmp);   // disable compiler warning
	//reset rx fifo
	bUART(ndev, oUFCON) |= UFCON_RX_REQ;
	return 0;
}

inline static int serial_flush_output(int ndev)
{
